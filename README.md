...
Если вы подключаете скрипт сразу на страницу, то добавьте инициализацию модуля в событие `onload`.

```html
<script src="..." onload="codexSpecial.init({});"></script>
```
...
Можете не показывать текст вовсе, задав `lang: 'none'`.

## Самостоятельная сборка модуля

Вы можете внести изменения в исходный код модуля, если это необходимо. Для этого вам потребуется установленный [Node.js](https://nodejs.org/en/).

Склонируйте репозиторий или скачайте архив.

<!-- Картинка (гифка?) с правым верхним углом страницы репозитория -->

Перейдите в директорию с содержимым репозитория и установите необходимые для разработки пакеты.

```shell
npm i --save-dev
```

Внесите изменения в исходный код модуля, находящийся в директории `src`.

Запустите Webpack, который будет следить за изменениями в исходных файлах модуля и автоматически обновит файл-сборку `codex-special.min.js`.

```shell
npm run build
```

Чтобы проверить работоспособность модуля, откройте файл `index.html`.
